export const standardDistribution = (
  ...
  //         and (prev.chord == Chord.V and next.chord == Chord.VI)
  //     ):
  //         # Sec. 17: V->VI の連結
  //         # I が重複するため例外
  //         pass
  //     else:
  //         chord_degs = chord_degrees(next.chord)
  //         if next.chord in {
  //             Chord.I,
  //             Chord.II,
  //             Chord.IV,
  //             Chord.V,
  //             Chord.VI,
  //         } or next.chord in {Chord.I_2, Chord.IV_2, Chord.V_2}:
...
// def standard_leading(prev: Harmony, next: Harmony) -> bool:
...
//     # 保留
//     if prev.chord == Chord.II and next.chord == Chord.V:
//         # Sec. 17: II->V の連結
//         # II は保留せず VII に下行させる
//         if prev.ten % 7 == 1 and next.ten % 7 != 6:
//             return False
...
//     else:
//         # (1) 上3声の共通音は保留する
//         if prev.ten in [next.bas, next.ten, next.alt, next.sop]:
//             if prev.ten != next.ten:
//                 return False
//         if prev.alt in [next.bas, next.ten, next.alt, next.sop]:
//             if prev.alt != next.alt:
//                 return False
//         if prev.sop in [next.bas, next.ten, next.alt, next.sop]:
//             if prev.sop != next.sop:
//                 return False

//     # (2) 配分一致
//     if not(prev.chord == Chord.V7_2_ and next.chord in {Chord.I, Chord.I_1}): # p81
//         if (next.sop - next.ten) > 7 and (prev.sop - prev.ten) < 7:
//             return False
//         if (next.sop - next.ten) < 7 and (prev.sop - prev.ten) > 7:
//             return False
