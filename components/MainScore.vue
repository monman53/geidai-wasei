...
<template>
  <div>
    ...
    <!-- <svg width="800" height="200" viewBox="-200 -50 400 100"> -->
        <!-- 五線 -->
        <line v-for="i in [0, 1, 2, 3, 4]" :key="i" x1="0" :y1="-u*4-u*i" :x2="width" :y2="-u*4-u*i" stroke="black" :stroke-width="bravuraMetadata.engravingDefaults.staffLineThickness * u"/>
        <line v-for="i in [0, 1, 2, 3, 4]" :key="i" x1="0" :y1="u*4+u*i" :x2="width" :y2="u*4+u*i" stroke="black" :stroke-width="bravuraMetadata.engravingDefaults.staffLineThickness * u"/>
        <!-- 複縦線 -->
        ...
        <!-- ヘ音記号 -->
        <text :x="u" :y="u*4+u*4-3*u" class="bravura-text">&#xe062;</text>
        <g v-for="(h, h_idx) in harmonies" :key="h_idx">
          <!-- bas -->
          <text :x="24 + 16*h_idx" :y="u*3 + -2 * h.bas" class="bravura-text">&#xe1d4;</text>
          <!-- ten -->
          <text :x="24 + 16*h_idx" :y="u*3 + -2 * h.ten" class="bravura-text">&#xe1d3;</text>
          <!-- alt -->
          <text :x="24 + 16*h_idx" :y="-u*3 + -2 * h.alt" class="bravura-text">&#xe1d4;</text>
          <!-- sop -->
          <text :x="24 + 16*h_idx" :y="-u*3 + -2 * h.sop" class="bravura-text">&#xe1d3;</text>
        </g>
    </svg>
  </div>
</template>
